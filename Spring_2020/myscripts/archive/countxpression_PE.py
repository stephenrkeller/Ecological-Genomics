...
def countxpression(infilename, threshold, lengththresh, zeroforheader1forno, tosort, outfilename, countsoutputname):
	...
	for line in IN:
		...
		if cols[0][0] != '@' : #to skip past header
			if 'SM:i:' in line:
				...
				columntocount=14 #this might vary depending on mapping software; we want columntocount to be "X0:i:#"
			if 'X0:i:' not in line : #this is to count reads that do not contain the X0 flag and can thus be considered unaligned
				notaligned+=1
				totreads+=1
			elif cols[columntocount].split(':')[0] != "X0" : #this is to ignore (and count) any lines that have the X0 flag in the wrong place.
				notaligned+=1
				totreads+=1
				wrongformat+=1
	...
	if tosort=='text':
		if int(zeroforheader1forno) == 0:
			countsout.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' % ('Filename', 'Total#Reads', 'NumContigsMatched', 'NumUnaligned', 'NumAligned', 'NumMultiAligned', 'NumSingleAligned', 'NumQualSingles', 'Wrongformat', 'PropQualAligned'))
		countsout.write('\n%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%.3f' % (infilename, totreads, contigsmatched, notaligned, aligned, multi, single, goodmaps, wrongformat, propqualaligned))
