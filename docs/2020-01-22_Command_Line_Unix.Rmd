...
- For the first part of the semester, we're going to analyze some unpublished exome-capture Illumina data sequenced from red spruce trees.  For this project, my lab collected needle tissue and cones from trees across the Appalachian mountains, from Tennessee, USA to New Brunswick, Canada. 

![](https://github.com/stephenrkeller/Ecological_Genomics/blob/master/Spring_2020/mydata/Figure2_Capblancq_etal_EvolApps.png?raw=true) 

- If it looks like this was a lot of hard work, well, you've got it right!
![](https://enjoyburlington.com/wp-content/uploads/sites/10/2018/09/credit_Sonia-DeYoung.jpg)
...
- The `fastq` dir contains the paired-end Illumina sequence files, while the `metadata` dir contains the collection info.  Navigate inside the `metadata` folder and you should see a file called `RS_Exome_metadata.txt`.  Let's use the `head` command to peek at the first 10 lines:
...
- The headers should be self-explanatory.  Note the column 'Region' -- this refers to the geographical and genetic cluster that each individual belongs to (see map above):  the "Core" (C), "Margin" (M), or "Edge" (E).  The entire dataset is part of a larger NSF-funded project on the population genomics of climate adaptation in red spruce.  For our class work, we're going to be focusing on just the Edge samples.  Bash gives us a convenient way to search within our file and pull out just the rows of data that match a term of interest (in this case, we'll search for "E").  We do this using a "generalized regular expression, also known as the `grep` command: 
...
```bash
[srkeller@pbio381 project_data]$ cd ~/Ecological_Genomics/mydata/
[srkeller@pbio381 mydata]$ ll
total 4
