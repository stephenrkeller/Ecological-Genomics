...
- For the first part of the semester, we're going to analyze some unpublished exome-capture Illumina data sequenced from red spruce trees.  For this project, my lab collected needle tissue and cones from trees across the Appalachian mountains, from Tennessee, USA to New Brunswick, Canada. We've placed a text file containing some metadata information on the samples that were sequenced under a shared space on the server. The path to this shared space is: 

    - `/data/project_data/RS_ExomeSeq`   Try using `cd` to navigate over to this location. Then `ll` to show its contents. You should see something like this:
...
- First let's look at what's in the `metadata` folder:

```
[srkeller@pbio381 metadata]$ ll
total 20
-rwxr-xr-x. 1 srkeller users 18971 Jan 21 14:05 RS_Exome_metadata.txt
``` 
