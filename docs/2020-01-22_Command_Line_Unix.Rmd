...
- We're going to use the Github repos we just created to help us stay organized with files moving back and forth between the server and our local machines.  The server is able to run git at the command line, which takes a little getting used to, but is actually quite easy.  The basic commands all start with calling the program `git` followed by the type of command you want to do.  First, we're going to make a copy (i.e., clone) of our repo that's on Github (up in the cloud!) and install it locally in our home directory on the server.  You'll only have to do this once!  

- First, go to your Github account in your web browser and navigate to your repo for Ecological Genomics.  Click the green "Clone or download" button on the right, and then click the icon of the clipboard.  The web address to your repo will be copied temporarily into memory, which you'll then paste in the command below on your terminal shell.  Your address should look like mine but with your username and repo name, and ending in .git

```
git clone https://github.com/stephenrkeller/Ecological_Genomics.git
```

- We now have a clones copy of our repo on the server in your home directory. Now, all your work between the server and your laptop can be synchronized and kept up to date!  

- Now, try to navigate into your repo at the command line using the `cd` command

```
[srkeller@pbio381 ~]$ cd Ecological_Genomics
```
...
- You'll notice that I've got some extra folders in my output from previous work, whereas you will may only see your notebook.
...
- You should make a new folder (aka, directory) using the `mkdir` command. Name this folder "mydata"

```
[srkeller@pbio381 ~]$ mkdir mydata
```

- Use the `ll` command to confirm its there, then make two more folders named "myscripts" and "myresults". Then use the `ll` command to list the folders again 

- Now we all have the same tidy organizational structure that we can use throughput the semester for our scripts, data, and results.  Speaking of data, let's go get some!

- For the first part of the semester, we're going to analyze some unpublished exome-capture Illumina data sequenced from [red spruce (*Picea rubens* Sarg.)](https://en.wikipedia.org/wiki/Picea_rubens) trees.  Red spruce is a long-lived coniferous tree that grows in cool moist habitats like high elevations in the mountains or along the coast of Atlantic Canada.  For this project, my lab collected needle tissue and cones from trees across the Appalachian mountains, from Tennessee, USA to New Brunswick, Canada. 
...
- If it looks like this was a lot of hard work, well, you've got it right!


- We've placed the raw sequence files and a text file containing some collection metadata under a shared directory on the server. The path to this shared space is: `/data/project_data/RS_ExomeSeq`   

- Try using `cd` to navigate over to this location. Then `ll` to show its contents. You should see something like this:
...
```
- The `fastq` dir contains the paired-end Illumina sequence files, while the `metadata` dir contains the collection info.  Navigate inside the `metadata` folder and `ll`, you should see a file called `RS_Exome_metadata.txt`.  

- We don't want to open and make changes to this file in the shared space, because we don't want to have our edits affect the rest of the group. (Also, notice that you don't have "write" (w) permissions, so you can't anyway!)  Best practices are to first make a copy of this file over to our home directory and put it inside your "mydata" folder. Use the `cp` command, followed by the filename, and the path to your destination (remember the ~ signals your home directory, and each subdirectory is then separated by a /):
...
```
[srkeller@pbio381 project_data]$ cd ~/Ecological_Genomics/Spring_2020/mydata/
[srkeller@pbio381 mydata]$ ll
total 20
drwxr-xr-x. 2 srkeller users    56 Jan 21 17:56 misc
-rwxr-xr-x. 1 srkeller users 18971 Jan 21 17:05 RS_Exome_metadata.txt
```

- Let's use the `head` command to peek at the first 10 lines:

```
[srkeller@pbio381 metadata]$ head RS_Exome_metadata.txt
Pop	Tree	Family	State	Region	Date	Latitude	Longitude	Elevation	DBH
...
AB	16	AB_16	TN	E	09002016	35.53882	83.49534	1738	44.0
AB	18	AB_18	TN	E	09002016	35.53945	83.49375	1769	68.2
ALB	01	ALB_01	ME	C	11112017	44.30670	70.84019	455	28.1
ALB	02	ALB_02	ME	C	11112017	44.30564	70.84081	524	11.3
ALB	03	ALB_03	ME	C	11112017	44.30359	70.84096	523	19.9
ALB	05	ALB_05	ME	C	11112017	44.30182	70.83729	573	11.9
``` 
...
AB      16      AB_16   TN      E       09002016        35.53882        83.49534        1738    44.0
AB      18      AB_18   TN      E       09002016        35.53945        83.49375        1769    68.2
BFA     01      BFA_01  WV      E       09282017        38.47660        79.72290        1348    21.8
BFA     02      BFA_02  WV      E       09282017        38.47666        79.71849        1325    14.6
...
[srkeller@pbio381 mydata]$ ll
total 28
-rw-r--r--. 1 srkeller users  6123 Jan 21 18:08 Edge_only.txt
drwxr-xr-x. 2 srkeller users    56 Jan 21 17:56 misc
-rwxr-xr-x. 1 srkeller users 18971 Jan 21 17:05 RS_Exome_metadata.txt
[srkeller@pbio381 mydata]$
```
...
- One of the most useful aspects of UNIX is the ability to take the output from one command and use it as standard input (termed 'stdin') into another command without having to store the intermediate files. Such a workflow is called "piping", and makes use of the pipe character (|) located above the return key to feed data between programs.
  - Example: Say we wanted to know how many unique populations are contained within the Edge region.  We can use `grep` to do the search and use the pipe (|) to send the results of grep to the `wc -l` command, which will tally up the number of lines.
...
-Looks like there's 110 sample individuals in the Edge region.  We can get fancier...let's find the # of unique populations that are contained in the edge.  We can do this by piping together multiple commands all in a row.  New commands are: `cut` to get just the column containing the population code (the first col), and `uniq` to collapse the number of rows to just 1 per (unique) population.  Here's the whole thing in one go, but in class we'll build this from the ground up:
...
- And N=23 unique populations.  What are they?  (hint:  just remove the `wc -l` command from the end of the pipe, and it will print them to the screen)
...
- Now try saving the unique populations to a new file that you name "Edge.pops"

- What if we want to get info for just individuals of a particular population. There's a way to do this quickly using the wildcard character "*". With the wildcard, the "*\*" takes the place of any character, and in fact any length of characters. For example, do a `grep` search for all individuals that command.
